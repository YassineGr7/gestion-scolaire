...
@section('content')
    <div class="flex  flex-col justify-center items-center h-screen">
        <h2 class="text-2xl font-semibold mb-2 ">Edit Teacher</h2>
        <form class="max-w-md mx-auto mt-3" method="POST" action="{{ route('update-teacher', ['id' => $teacher->id]) }}">
            @csrf
            @method('PUT')
            <div class="grid md:grid-cols-2 md:gap-6">
                <div class="relative z-0 w-full mb-5 group">
                    <input type="text" name="nom" id="floating_first_name"
                        ...
                        placeholder=" " value="{{ $teacher->nom }}" required />
                ...
                <div class="relative z-0 w-full mb-5 group">
                    <input type="text" name="prenom" id="floating_last_name"
                        class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-dark dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                        placeholder=" " value="{{ $teacher->prenom }}" required />
            ...
            <div class="relative z-0 w-full mb-5 group">
                <input type="tel" name="phone" id="floating_phone"
                    class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-dark dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                    placeholder=" " value="{{ $teacher->phone }}" required />
